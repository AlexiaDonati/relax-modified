...
export class AntiJoin extends Join {
	constructor(child: RANode, child2: RANode, condition: JoinCondition) {
		super(child, child2, '▷', 'relalg-operations-antijoin', condition, false, true);
	}
	...
	equalsJoin(node: RANode): boolean {
		if (node instanceof AntiJoin) { // same type
			// the order of the children does matter for the anti join operation
