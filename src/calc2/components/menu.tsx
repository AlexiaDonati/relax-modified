			...
			<div className="container">
				<div className="row">
								...
								<li key={`${headline}`}>
									...
									<ul>
										{groups.map((group: any, i: any) => {
											...
											const path = `/calc/group/${groupInfo.source}/${groupInfo.id}/${groupInfo.filename}/${groupInfo.index}`;
					...
					<div className="col-md-6 align-text-top align-top">
						...
						<input type="text" className="form-control gist-load-input" placeholder="" data-i18n="[placeholder]calc.menu.load-gist-insert-placeholder" size={32} onChange={(event) => { this.gistLink = '/calc/group/gist/' + event.target.value; }} />
						...
						<ul>
							{
								recentlyUsedGroups.reverse().map((el: any) => 
									<li key={el.name}>
										<NavLink to={'/calc/group/gist/'+el.group.groupInfo.id} onClick={()=>{this.props.datasetLoaded(); document.location.href = '/calc/group/gist/'+el.group.groupInfo.id; }}>{el.name}</NavLink>
									</li>,
								)
							}
