			...
			<div className="container">
				<div className="row">
					...
					<div className="col-md-6 align-text-top align-top">
						...
						<input type="text" className="form-control gist-load-input" placeholder="" data-i18n="[placeholder]calc.menu.load-gist-insert-placeholder" size={32} onChange={(event) => { this.gistLink = '/relax-modified/calc/group/gist/' + event.target.value; }} />
						...
						<ul>
							{
								recentlyUsedGroups.reverse().map((el: any) => 
									<li key={el.name}>
										<NavLink to={'/relax-modified/calc/group/gist/'+el.group.groupInfo.id} onClick={()=>{this.props.datasetLoaded(); document.location.href = '/relax-modified/calc/group/gist/'+el.group.groupInfo.id; }}>{el.name}</NavLink>
									</li>,
								)
							}
						...
						<p><T id="calc.menu.create-own-dataset-text" /> <Link to="relax-modified/help#tutorial-maintainer"><T id="calc.menu.create-own-dataset-text-link" /></Link></p>
