...
type Props = {
	...
	execFunction(self: EditorBase, text: string, offset: CodeMirror.Position): { result: JSX.Element },
	execTestsFunction?(self: EditorBase, text: string, offset: CodeMirror.Position, exerciseGroup: boolean, exerciseGroupIndex: number): { result: JSX.Element },
	...
	exerciseReference: Exercise | undefined,
...
export class EditorBase extends React.Component<Props, State> {
	...
	resetHinter() { // should be called after group change
		this.hinterCache = {
			hints: [],
			hintsFromLinter: [],
			changed: true,
	...
	async verify() {	
		...
		const exercise = this.props.exerciseReference;
		if(!exercise){
			throw new Error('reference exercise is not initialized');
		}
		...
		this.setState({ // show spinner while verifying
			...
			let referenceQuery = exercise.reference;
			...
			try {
				...
				if(exercise.verificationGroups.groups && this.props.execTestsFunction){
					
					for(let i = 0 ; i < exercise.verificationGroups.groups.length ; i++) {
						// execute the queries
						const execTestResult = this.props.execTestsFunction(this, query, {line: 0, ch: 0}, true, i).result;
						const referenceTestResult = this.props.execTestsFunction(this, referenceQuery, {line: 0, ch: 0}, true, i).result;

						// get the resulting tables
						const queryTestResult = result(execTestResult.props.root, editor.getOption('mode') !== 'bagalg');
						if(!queryTestResult) {
							throw new Error(`could not compute result for the student query`);
						}

						const referenceQueryTestResult = result(referenceTestResult.props.root, editor.getOption('mode') !== 'bagalg');
						if(!referenceQueryTestResult) {
							throw new Error(`could not compute result for the reference query`);
						}
	...
	// region Inline Relation Markers
